let curr_info = [
		{
			...
			abb_name: "BTC",
			img_url:
				"https://res.coinpaper.com/coinpaper/bitcoin_btc_logo_62c59b827e.png",
		},
		{
			curr_name: "Ethereum",
			abb_name: "ETH",
		...
		},
		{
			curr_name: "Shiba Inu",
			abb_name: "SHIB",
			img_url: "https://cryptologos.cc/logos/shiba-inu-shib-logo.png",
		},
		{
			curr_name: "Sweat Economy",
			abb_name: "SWEAT",
			img_url:
				"https://framerusercontent.com/images/FYhfhb3ysVXGh3ajfmgFYYbpU.webp",
		},
		{
			curr_name: "NEAR Protocol",
			abb_name: "NEAR",
			img_url:
		...
		},
		{
			...
			abb_name: "MATIC",
			img_url:
				"https://pbs.twimg.com/profile_images/1781425963265327104/TB5fMI9O_400x400.jpg",
		},
		{
			curr_name: "Bonk",
			abb_name: "BONK",
			img_url: "https://cryptologos.cc/logos/bonk1-bonk-logo.png",
		},
		{
			curr_name: "TRON",
			abb_name: "TRX",
			img_url: "https://cdn-icons-png.flaticon.com/512/12114/12114250.png",
		},
		// {
		// 	curr_name: "Hamster Kombat",
		// 	abb_name: "HMSTR",
		// 	img_url: "https://hamsterkombat.me/hamster-kombat-coin.png",
		// },
	],
...
for (let i = 0; i < curr_info.length; i++) {
	...
	fetch(
		`https://api.coinbase.com/v2/exchange-rates?currency=${curr.abb_name}`,
		{}
	).then((res) =>
		res.json().then((data) => {
...
document
	...
	.addEventListener("click", (_) => calc_container.classList.remove("active"));
...
function AddCardToContainer(img_url, curr_name, abb_name, curr_price) {
    ...
    <div class="card curr-${abb_name.toLowerCase()}">
        ...
        <div class="info">
            ...
            <p>~$${
							parseFloat(curr_price) < 0.0001
								? parseFloat(curr_price).toFixed(6)
								: parseFloat(curr_price).toFixed(4)
						}</p>
        </div>
	...
	curr_card_list.forEach((card) => {
		...
		card_cont.querySelector("span.calc").addEventListener("click", (_) => {
			...
			document.querySelector(
				".calc-card .header"
			).innerHTML = `<img src="${curr_logo}" alt="">
...
function RetrivePricesForCal(curr_abb) {
	fetch(
		`https://api.coinbase.com/v2/exchange-rates?currency=${curr_abb}`,
		{}
	).then((res) =>
		res.json().then((data) => {
